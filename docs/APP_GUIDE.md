...
현재 구현 기준은 16개 직무군, 96개 세부 직무, 127개 교육자료다.
...
전공과 세부 직무의 연결은 과대평가하지 않는다. 모든 추천 직무는 다음 3단계 중 하나로 설명한다.

| 단계 | 의미 | 노출 기준 |
|---|---|---|
| 전공 직결 | 전공 지식이 반복 업무와 직접 맞닿는 직무 | 전체 보기와 산업 필터에서 우선 노출 |
| 전공 확장 | 프로젝트와 보완 학습으로 진입 가능한 직무 | 전체 보기와 산업 필터에서 함께 노출 |
| 도전 직무 | 별도 기초 언어와 도구 확인이 먼저 필요한 직무 | 산업, 검색, 직무군 필터를 좁혔을 때만 노출 |

산업 필터는 직무군 태그만으로 판단하지 않는다. 선택 산업이 `all`이 아니면 세부 직무의 `industries`에 해당 산업이 있을 때만 통과시킨다. 예를 들어 AI 산업 화면에서 자동차·화학 기본 직무가 `all` 태그만으로 상단에 올라오면 안 된다.
...
`candidate`, `reviewNeeded`, `broad` 자료는 보조 후보로 남길 수 있지만 Starter Pack과 상위 추천을 검수 자료보다 앞지르면 안 된다. 추천 메인 정렬은 reviewed/verified 비후보 자료를 먼저 세우고, 검수 자료가 부족한 영역에서만 후보 자료를 뒤에 채운다. 주차별 상세 추천과 실습 후보 정렬도 같은 신뢰도 기준을 따른다. UI에서는 후보 상태를 명확히 표시한다.
...
- 코멘토 생산관리 핵심 실무
- Quality-One FMEA
- iSixSigma FMEA와 Quality-One 8D
...
- KiCad PCB 설계 공식 문서
- TI Power Management 공식 개요
- SparkFun 회로 기본 핵심 단원
- TI Precision Labs
- EMC/EMI 기초 자료
...
우선 교육은 KDCC 데이터센터 구축·운영 자료, Simscape Electrical, Simscape Fluids, 계측·시험 자료를 중심으로 연결한다. 완성할 산출물은 전력 단선도 검토표, 냉각 용량 계산표, BMS/DCIM 알람 원인분석표, 장애 대응 체크리스트다.
