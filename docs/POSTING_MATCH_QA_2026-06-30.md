...
- `오늘 시작할 1개` 카드의 추천 근거 라벨이 비어 있지 않아야 한다.
...
| 생산 데이터 분석 공고 - 기초 체크 완료 | 생산 데이터 분석 담당 | 68% | Machine Learning Onramp | PASS |
...
- 생산 데이터 분석 케이스는 오늘 시작 자료뿐 아니라 추천 근거도 달라진다. 기초 미체크 상태는 `보완 역량`, `자료 신뢰`가 잡히고, 기초 체크 완료 상태는 `공고 신호`, `체크 반영`, `자료 신뢰`가 잡힌다.
...
- 생산 데이터 분석 케이스는 역량 체크 상태에 따라 오늘 시작 자료가 `NIST Control Charts`에서 `Machine Learning Onramp`로 바뀐다. 다만 실제 지원자의 Python/SQL 숙련도까지 반영하려면 추가 체크 항목이나 수동 QA가 필요하다.
...
- 생산 데이터 분석처럼 통계 기초와 Python/SQL 실습 사이의 우선순위가 학생 역량 체크에 따라 달라지는 케이스를 실제 공고 원문으로 추가 확인한다.
