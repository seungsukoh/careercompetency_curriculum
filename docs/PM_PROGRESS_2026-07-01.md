...
사용자 요청에 따라 색상 토큰이 실제 화면 의미와 맞게 반영되어 있는지, 그리고 가독성을 해치지 않도록 여백이 충분한지 다시 점검했다.

발견:

- 기존 CSS에서 `--green`, `--danger`, `--teal`, `--violet`이 모두 네이비 계열로 수렴해 완료/주의/위험/보조 정보의 시각적 구분이 약했다.
- 헤더 H1에 음수 자간이 남아 있어 장기적으로 텍스트 가독성 규칙과 맞지 않았다.
- 워크플로우 단계, 직무 카드, 배지, 교육 요약 카드, 주차별 자료 카드의 간격이 기능적으로는 통과했지만 조밀하게 보일 수 있었다.

반영:

- `styles.css`의 루트 색상 토큰을 정리했다.
  - primary: 네이비
  - success: 절제된 그린
  - warning: 앰버
  - danger: 딥 레드
  - teal/violet: 보조 상태 구분용 저채도 색
- 완료, 저장, 체크 완료 상태는 `--green`을 사용하고, 위험/초기화 버튼은 `--danger` 계열을 사용하도록 의미 색상을 분리했다.
- 워크플로우, 직무 카드, 배지, 추천 교육 카드, 역량 체크, 주차별 교육 카드의 `gap`과 `padding`을 소폭 늘려 스캔성을 높였다.
- H1 음수 자간을 제거해 `letter-spacing: 0`으로 맞췄다.
- `scripts/audit-ui-colors.mjs`와 `npm.cmd run audit:colors`를 추가해 색상 토큰 중복, 주요 대비, 음수 자간을 자동 검증하도록 했다.
- `npm.cmd run check`에 색상 감사를 포함했다.

검증 결과:

- `npm.cmd run audit:colors` PASS
  - checkedTokens 20
  - contrastPairs 11
  - semantic colors: primary `#0e2750`, success `#17634a`, warning `#965a08`, danger `#9b2f35`, teal `#0f6674`, violet `#5f4b8b`
- `npm.cmd run check` PASS
- `npm.cmd run qa:ux` PASS
  - 13개 시나리오 × 2개 뷰포트 = 26개 샘플
  - failures 0
  - 스크린샷/리포트: `tmp/browser-ux-samples/run-19532-1782906835878`

판단:

- 색상은 한 가지 네이비 계열로만 읽히던 상태에서 의미별로 구분되도록 개선됐다.
- 여백은 화면 밀도를 크게 낮추지 않는 선에서 핵심 카드와 추천 자료 영역 위주로 보강했다.
- QA 기준으로 데스크톱/모바일 overflow나 추천 중복 회귀는 발견되지 않았다.
