...
1. 학생 관점 문구 보정 변경을 커밋하고 푸시한다.
2. Cloudflare Pages 재배포 후 `소재개발과 바로 연결됩니다` 문구가 배포본에 반영됐는지 짧게 확인한다.
3. 파일럿 전에는 "전공과 직무가 억지 연결처럼 보이지 않는가", "추천 교육이 직무역량을 보완한다고 느껴지는가", "직무와 학과와 교육이 화면에서 서로 다른 개념으로 읽히는가"를 별도 질문으로 수집한다.
